...
The entire manufacturing process can be broadly divided into three main phases: design \& development, production \& testing, and sales \& service~\cite{Zhou2019}.
%三个阶段分别有不同的任务和数据。
Each of the three phases has its own tasks and data.
%在设计研发阶段，除了产品样式及功能的设计研发之外，还包括工厂、店铺的选址规划。
In design \& development phase, in addition to product appearances and functions, it also includes the site selection and construction planning of factories and stores.
%在生产测试阶段，原材料配送顺序，工作人员及设备排班，产品质量和工厂流水线的监测分析等都是研究的热门方向。
In production \& testing phase, raw material distribution sequence, staff and equipment scheduling, product quality and factory line monitoring and analysis are all hot directions of research.
%在销售服务阶段，产品宣传和广告设计、销售数据及反馈意见分析等同样非常重要。
In sales \& service phase, product promotion and advertising design, sales data and feedback analysis are also very important.
...
%可视化和可视分析有多种途径将海量信息直观高效的转换成知识并进行推理。
...
Moreover, through analysis and comparison of related work, we identify trends and recent developments in multimedia data visualization and visual analytics in manufacturing.
